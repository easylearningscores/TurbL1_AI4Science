...
print("\n========== Loading Model Weights ==========")
epoch = 250
best_model_path = os.path.join(checkpoint_dir, f"{backbone}_epoch_{epoch}.pth")
...
# ================== Result Saving ==================
print("\n========== Saving Results ==========")
np.save(os.path.join(result_dir, f"{backbone}_{epoch}_initial_conditions.npy"), all_inputs)
np.save(os.path.join(result_dir, f"{backbone}_{epoch}_predictions.npy"), all_outputs)
np.save(os.path.join(result_dir, f"{backbone}_{epoch}_ground_truth.npy"), all_targets)
